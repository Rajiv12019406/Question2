function highlightContent(htmlContent, plainText, plainTextPosition) {
  plainTextPositions.sort((x, y) => y.start - x.start);

  for (const position of plainTextPosition) {
    const highlightedWord = plainText.substring(position.start, positionnode htm.end);
...
const ans = highlightContent(htmlContent, plainText, plainTextPositions);
console.log(ans);
